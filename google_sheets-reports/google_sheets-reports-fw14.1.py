...
try:
  r = requests.get(webadmin_api+'clients/groups_tree?limit=5000', headers=authheader)
  groupstree_data = r.json()['groups_hierarchy']
except Exception as e:
...
def group_path(group_id):
  for group in groupstree_data:
    if group['id'] == group_id:
      prettypath = ''
      for element in group['path'][1:]:
         prettypath = prettypath +'/'+ element
      return(prettypath)
...
for group in groupstree_data:  # as long as there's a 'next' URL, keep fetching more results, and append results to 'rows' 
  try:
    r = http.get(webadmin_api + 'clients/?parent_id=%s' % group['id'], headers=authheader)
    resp=r.json()
